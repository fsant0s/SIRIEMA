...
def load_data_from_folder(
    ...
    Args:
        ...
        train_csv_name (str): Train dataset name. Default is 'train.csv'
        val_csv_name (str): Validation dataset name. Default is 'val.csv'
        test_csv_name (str): Test dataset name. Default is 'test.csv'
        num_random_samples (int): number of samples to train, val, and test the model. To use the whole dataset set it to zero
    ...
    """
    train_df = pd.read_csv(join(folder_path, train_csv_name))
    test_df = pd.read_csv(join(folder_path, test_csv_name))
    if exists(join(folder_path, val_csv_name)):
        val_df = pd.read_csv(join(folder_path, val_csv_name))
    else:
        val_df = None

    if num_random_samples:
        train_df = train_df.sample(n=num_random_samples)
        val_df = val_df.sample(n=num_random_samples)
        #test_df = test_df.sample(n=num_random_samples)
...
def load_train_val_test_helper(
    ...
    """
        Modificitaion done by fillipe santos
        Adding: categorical_cols != None and len(categorical_cols)
    """
    if categorical_cols != None and len(categorical_cols) and (categorical_encode_type == "ohe" or categorical_encode_type == "binary"):
        ...
        data_df = pd.concat([data_df.reset_index(drop=True), cat_df.reset_index(drop=True)], axis=1)
